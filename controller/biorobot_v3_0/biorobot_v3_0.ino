...
void loop() {
  ...
  /* X AXIS */
  if (xmov < x && millis() - xmillis >= timeout && e != 1 && xlimit != LOW && x2limit != LOW) {
    xmov++;
    xmillis = millis();
    xstate = !xstate;
    xstarted = 1;
  ...
  /* Y AXIS */
  if (ymov < y && millis() - ymillis >= timeout && e != 1 && ylimit != LOW && y2limit != LOW) {
    ymov++;
    ymillis = millis();
    ystate = !ystate;
    ystarted = 1;
  ...
  /* Z AXIS */
  if (zmov < z && millis() - zmillis >= timeout && e != 1 && xstarted == 0 && ystarted == 0 && zlimit != LOW && z2limit != LOW) {
    zmov++;
    zmillis = millis();
    zstate = !zstate;
    zstarted = 1;
