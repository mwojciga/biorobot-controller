...
void loop() {
  ...
  if(xlimit == LOW || x2limit == LOW ) {
    ...
    digitalWrite(A4, xdir);
    digitalWrite(A5, LOW);
    digitalWrite(12, LOW); // enable motors
    digitalWrite(11, HIGH); // release stop.
    // porusz sie o 10 krokow
    for (i = 0; i <= 5; i++){
    ...
    digitalWrite(12, HIGH); // block motors
    digitalWrite(11, LOW); // stop.
    printCoordinates(xlimit, x2limit, ylimit, y2limit, zlimit, z2limit, xpos, ypos, zpos, waited, e);
  }
